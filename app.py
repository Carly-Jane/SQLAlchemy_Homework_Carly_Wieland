...
def homepage(): #List all routes that are available
    return (
        ...
        f"JSON list of the minimum temperature, the average temperature, and the max temperature for a given start date<br/>"
        f"Calc TMIN, TAVG, and TMAX for all dates greater than and equal to the start date.<br/>"
...
def precipitation(): # JSON representation of precipitation dictionary.
    ...
    precipi = session.query(measurement.date, measurement.prcp).\
        filter(measurement.date > query_date).\
        order_by(measurement.date).all()
    
    prcp_rows = [{"Date": result[0], "Precip": result[1]} for result in precipi]   
